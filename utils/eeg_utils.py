...
def load_and_preprocess_eeg(file_path, file_type='set'):
    """
    Carga y preprocesa archivo EEG (.set o CSV).
    - Valida canales estándar.
    - Aplica preprocesamiento básico: filtrado bandpass (0.5-45 Hz), remoción de artifacts simples.
    - Retorna datos raw preprocesados (MNE Raw o DataFrame).
    """
    try:
        if file_type == 'set':
...
def extract_spectral_features(raw):
    """
    Extrae features espectrales de datos EEG raw (MNE Raw).
    - Calcula PSD (Power Spectral Density) por canal.
    - Extrae: abs_power, rel_power, ratios (e.g., theta/alpha), entropy por banda y canal.
    - Retorna DataFrame con features (columnas como 'abs_power_delta_Fp1', etc.).
    """
    try:
